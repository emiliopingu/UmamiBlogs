...
<body>
    <header>
        <nav class="mi-navegacion">
            ...
            <ul class="mi-navegacion-items">
                <li>
                <a class="btn btn-primary" href="{{ route('recetas_tailandesas.index') }}"> Volver a la seleccion de recetas</a>
                </li>
            </ul>
        </nav>
    </header>
    ...
    <div class="centro">
    <div>
        <img src="{{$recetas_tailandesa->imagen}}" class="imagenReceta">
    </div>
    <h2>{{$recetas_tailandesa->nombre}}</h2>
    <br>
    <p><strong>Ingredientes : </strong>{{$recetas_tailandesa->ingredientes}}</p>
    <br>
    <h4>Como elaborarlo</h4>
    <div class="parrafo">
        <p><strong></strong>{{$recetas_tailandesa->descripcion}}</p>
    </div>
    ...
    <div class="valoracion">
        <h4>Valoración</h4>
        <p>{{$recetas_tailandesa->valoracion_text}} </p>
        <br>
        <p>Valoración : {{$recetas_tailandesa->valoracion_int}}</p>
    </div>
    <br>
    <a class="btn btn-primary" href="{{ route('recetas_tailandesas.index') }}"> Volver a la seleccion de recetas</a>
</div>
